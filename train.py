...
for e in range(opt.epochs):
    ...
    for i, (input_img, label_img, ori_img, hs_img) in enumerate(trainloader): #, hs_img
        ...
        for k in range(len(hist_img_list)):
            ...
            # maeloss = criterionL1(RGB_hs_img, gt)
            # percep_loss = perceploss(RGB_hs_img,gt)
            # mae_percep_loss = maeloss + percep_loss#
        ...
        if (e+1)%10 == 0 or (e+1+200)>150 :
            torch.save(model, os.path.join(opt.save_dir,"Histoformer-PQR_{}_modifyloss.pth".format(e+1)))
            torch.save(net_g, os.path.join(opt.save_dir,"Histoformer-PQR_netG_{}_modifyloss.pth".format(e+1)))
            torch.save(net_d, os.path.join(opt.save_dir,"Histoformer-PQR_netD_{}_modifyloss.pth".format(e+1)))

     
# x2=range(0,opt.epochs)
# plt.figure(figsize=(15, 5))
# plt.subplot(121)
# plt.plot(x2, Loss_avg, '.-')
# plt.title("train") 
# plt.subplot(122)
# plt.plot(x2, Loss_avg_v, '.-')
# plt.title("val")
# # plt.savefig('Ours/Histoformer-PQR.jpg') 
# plt.savefig(os.path.join(opt.save_dir,'Histoformer-PQR_modifyloss.jpg'))